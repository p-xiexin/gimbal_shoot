	...
	while (1)
	{
		...
		for(const auto& rect : coneRects_red)
		{
			if(rect.height > min_size && rect.height < max_size && rect.width > min_size && rect.width < max_size)
			{
				...
				cv::rectangle(img_color, rect, cv::Scalar(0, 0, 255), 2);
			}
		}
		if(points_red.size() == 1)
		{
			std::cout << "red point: " << "(" << points_red[0].x << ", " << points_red[0].y << ")  ";
			circle(img_color, Point(50, 40), 8, Scalar(0, 0, 255), -1);
		...
		if(type == 1)
		{
            ...
            putText(img_color, "Canny", Point(40, 40), FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 255), 1);
		}
		else if(type == 0)
		{
			cv::threshold(grayImage, edges, 0, 255, cv::THRESH_BINARY_INV | cv::THRESH_OTSU);
            putText(img_color, "OTSU", Point(40, 40), FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 255), 1);
		}
		...
		for (size_t i = 0; i < contours.size(); i++)
		{
			...
			if (approx.size() == 4)
			{
                ...
                // 绘制外接矩形
                rectangle(img_color, boundingRect, Scalar(255, 0, 0), 2);
				// 绘制矩形
				cv::polylines(img_color, approx, true, cv::Scalar(0, 255, 0), 2);
                ...
                cv::circle(img_color, Point(centerX, centerY), 5, cv::Scalar(226, 43, 138), -1);
